...
class LearnedPatternCategorizer(BaseCategorizer):
    ...
    Every manual correction in the UI is saved to the learned_rules table.
    Exact description match (case-insensitive), scoped per institution.
    Returns None if no learned rule matches.
    """
    ...
    def categorize(
        ...
        stmt = select(LearnedRule).where(
            and_(
                ...
                LearnedRule.description_pattern == description.lower().strip(),
            )
        )
        rule = self._session.execute(stmt).scalar_one_or_none()
